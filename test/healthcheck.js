...
describe('Healthcheck Check', () => {
  ...
  it('Requires the presence of a healthcheck', async () => {
    const orders = {
      path: 'streamliner/orders',
      contents: [
        'export NOT_A_HEALTHCHECK=nothing'
      ]
    }
    ...
    expect(results[0].problems[0]).to.equal(`**${orders.path}** - You must set a healthcheck, and it cannot be at \`/\``)
  });
  ...
  it('rejects healthchecks at /', async () => {
    ...
    expect(results[0].problems.length).to.equal(1);
    expect(results[0].problems[0]).to.equal('**/** - Your healthcheck cannot be at root.')
  ...
  it('rejects an empty healthcheck', async () => {
    ...
    expect(results[0].problems.length).to.equal(1);
    expect(results[0].problems[0]).to.equal('You must set a healthcheck, and it cannot be at `/`')
