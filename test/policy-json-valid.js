...
describe("policy.json is valid", () => {
  ...
  it("rejects policy.json with capitalization errors", async () => {
    ...
    expect(resource).to.deep.equal({
      ...
      problems: [
        suggest(
          "Capitalize this key",
          '      "Resource": "arn:aws-cn:secretsmanager:cn-north-1:868468680417:secret:dev/json_secret"'
        ),
      ],
      line: 15,
      level: "failure",
  ...
  it("requires all secrets in secrets.json to be present in policy.json", async () => {
    ...
    let deployment = {
      ...
      secretsJson: [
        ...
        },
        {
          ...
          valueFrom:
            "arn:aws-cn:secretsmanager:cn-north-1:868468680417:secret:dev/something_else:::",
        },
      ],
    };
    let results = (await policyJsonIsValid(deployment, context, inputs)).filter(
    ...
    expect(results[0]).to.deep.equal({
      ...
      path: "streamliner/policy.json",
      problems: [
        "Your secrets.json requests arn:aws-cn:secretsmanager:cn-north-1:868468680417:secret:dev/something_else-??????, but your policy does not allow access.",
      ...
      secretsJson: [
        {
          name: "MY_SECRET",
          valueFrom:
            "arn:aws-cn:secretsmanager:cn-north-1:868468680417:secret:dev/json_secret:example::",
        },
        {
          name: "MY_OTHER_SECRET",
          valueFrom:
        ...
        Statement: [
          {
            Effect: "Allow",
            Action: "secretsmanager:GetSecretValue",
            Resource: "arn:aws-cn:secretsmanager:cn-north-1:868468680417:secret:*", // supports wildcards
          },
          {
            Effect: "Allow",
            Action: [
    ...
    const policyJson = JSON.stringify(
      {
        ...
        Statement: [
          ...
          },
          {
            ...
            Resource: "arn:aws-cn:secretsmanager:cn-north-1:868468680417:secret:*", // doesn't care that this is the wrong resource type
          },
        ],
      },
      null,
    ...
    const deployment = {
      ...
      secretsJson: [
        ...
        },
        {
          name: "MY_OTHER_SECRET",
          valueFrom:
            "arn:aws-cn:secretsmanager:cn-north-1:868468680417:secret:dev/something_else:::abcdef",
        },
      ],
    };
