...
const debugs = [enableDebug, debug, logLevel];
...
async function noDebugInProd(deployment) {
  ...
  log.info(`No Debug In Production - ${deployment.ordersPath}`);
    ...
    debugs.forEach((regex) => {
      if (regex.test(line)) {
        result.problems.push(
          "Did you mean to leave this configured this way in production?"
        );
      }
    });
