...
async function templateCheck(orders, context) {
  ...
  const securityMode = getExportValue(orders.contents.join('\n'), 'SECURITY_MODE');
  if (!securityMode) {
    results.push({
      title: 'Missing SECURITY_MODE',
      problems: ["`SECURITY_MODE` is missing.\n\nIf this is intentional, add `unpublished` to your orders file."],
      line: 0,
      level: 'failure'
    });
    ...
    for (let i = 0; i < orders.contents.length; i++) {
      ...
      if (line.startsWith('export SECURITY_MODE=')) {
        results.push({
          ...
          problems: [`This cluster only supports the following security modes: **${expectedModes.join(', ')}**`],
          line: i+1,
          level: 'failure'
        });
      }
